...
The first crucial move is making sure your web app or WordPress site is prepped and ready to get tested on. I'm working over here with a VPS in Amsterdam rocking 1 Core and 2GB RAM, and I just finished installing a fresh [WordPress instance](https://wp.rokimiftah.me).

The next step is to install k6 and Node.js depends on the operating system you are using.
...
export default function () {
  http.get('https://wp.rokimiftah.me');
  sleep(1);
}
```
</CodeBlockFrame>
...
![](https://cdn.rokimiftah.me/blog/k6-load-testing-101/1.webp)
...
![](https://cdn.rokimiftah.me/blog/k6-load-testing-101/2.webp)
...
![](https://cdn.rokimiftah.me/blog/k6-load-testing-101/3.webp)
The `export default function () { ... }` function serves as the entry point for the test scenario. This function will be executed once for each virtual user. Inside this function, we call `http.get()` to make an HTTP GET request to the specified URL. In our example, we access the `https://wp.rokimiftah.me` URL.
...
![](https://cdn.rokimiftah.me/blog/k6-load-testing-101/4.webp)
...
5. Result Analysis
   - The test data reveals that the application under evaluation demonstrated the capability to sustain the simulated workload of 10 concurrent users without encountering any failures or errors, a commendable feat considering the substantial geographical separation between the testing location in Indonesia and the server's hosting site in Amsterdam. Nonetheless, the observed average response time of 609 milliseconds and the 95th percentile response duration of 1.32 seconds may be perceived as suboptimal, particularly when factoring in the network latency induced by the long-distance data transmission.

Through a thorough examination of the test data procured from k6, and by taking into consideration the substantial physical distance separating the testing environment from the server infrastructure, you can identify potential bottlenecks that may be hindering optimal performance. Equipped with these valuable insights, you can make informed decisions aimed at enhancing your application's responsiveness, which may involve exploring techniques to alleviate the adverse effects of network latency on the user experience for individuals accessing your application from geographically dispersed locations.
